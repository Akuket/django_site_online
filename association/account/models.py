...
class User(AbstractUser):
    ...
    def get_last_validate_payment(self):
        try:
            return \
                self.payments.filter(status="is_paid", subscribed_until__gte=datetime.date.today())\
                    .order_by("-date")[0]
        except IndexError:
            pass
        return None
    ...
    def get_last_validate_card(self):
        try:
            return self.card.filter(card_exp_date__gte=datetime.date.today(), card_available=True).order_by("-date")[0]
        except IndexError:
